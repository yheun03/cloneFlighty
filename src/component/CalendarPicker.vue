...
<template>
  <div class="fc-calendar">
    ...
    <div class="fc-calendar__grid">
      <span v-for="day in ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT']" :key="day" class="fc-calendar__weekday">{{
        day }}</span>
      <span v-for="(day, index) in days" :key="index">
        <button v-if="day" type="button" :class="{ 'is-marked': marked.includes(day), 'is-selected': selected === day }"
