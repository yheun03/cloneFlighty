...
<template>
  <div class="fc-performance">
    ...
    <div v-for="item in (showAll ? airlines : airlines.slice(0, 4))" :key="item.name" class="fc-performance__bar">
      <div><span>◉ &nbsp;{{ item.name }}</span><span>{{ item.percent }}% &nbsp;({{ item.count }})</span></div><i><b
          :style="{ width: `${item.percent}%` }"></b></i>
    </div>
    ...
    <div v-for="item in performance" :key="item.label" class="fc-performance__meter"><span>{{ item.label }}</span><i><b
          :style="{ width: `${item.value}%`, background: item.color }"></b></i><strong>{{ item.value }}%</strong></div>
    ...
    <div v-for="flight in ['KL 1251', 'KL 6410', 'KL 703', 'AY 911']" :key="flight" class="fc-performance__delay">
      <span>◉ {{ flight }}</span><b>47m</b><span>AMS-HEL</span></div>
  </div>
</template>
